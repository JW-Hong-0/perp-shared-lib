...
logger = logging.getLogger("VariationalAuth")
    ...
    async def _generate_signing_data_async(self, address: str) -> Dict[str, Any]:
        ...
        async with curl_requests.AsyncSession(impersonate=self._impersonate, timeout=self._http_timeout) as s:
            ...
            try:
                ...
                logger.error(
                    "Variational signing data JSON decode failed (status=%s). Body preview: %s",
                    status,
                    preview,
                )
                if "wants you to sign in with your Ethereum account" in full_text:
