from .base import MultiPerpDex, MultiPerpDexMixin
import asyncio
...
ENDPOINTS: Dict[str, Tuple[str, str]] = {
    ...
    "set_leverage": ("POST", "/api/settlement_pools/set_leverage"),
    "get_leverage": ("POST", "/api/settlement_pools/leverage"),
}
...
class VariationalExchange(MultiPerpDexMixin, MultiPerpDex):
    ...
    async def _set_leverage_raw(self, coin: str, leverage: int):
        method, path = ENDPOINTS["set_leverage"]
        payload = {"leverage": str(leverage), "asset": str(coin).upper()}
        return await self._request(method, path, json_body=payload, coin=coin)

    async def _get_leverage_raw(self, coin: str):
        method, path = ENDPOINTS["get_leverage"]
        payload = {"assets": [str(coin).upper()]}
        return await self._request(method, path, json_body=payload, coin=coin)
    ...
    async def set_leverage(self, symbol: str, leverage: int) -> Dict[str, Any]:
        ...
        await self.initialize_if_needed()
        coin = str(symbol).upper()
        # leverage must be reasonable integer e.g. 5, 10, 20
        res = await self._set_leverage_raw(coin, leverage)
        return res

    async def get_leverage(self, symbol: str) -> Dict[str, Any]:
        """
        Get current leverage setting for a specific asset.
        """
        await self.initialize_if_needed()
        coin = str(symbol).upper()
        res = await self._get_leverage_raw(coin)
        return res
